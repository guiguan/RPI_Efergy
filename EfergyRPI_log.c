...
//
// Compile:
//  gcc -O3 -lm -Wall -o EfergyRPI_log EfergyRPI_log.c
//
// Run:
//  rtl_fm -f 433.55M -s 200k -r 96k -A fast | ./EfergyRPI_log -d 1
//
//------------------------------------------------------------------------------
...
#define VOLTAGE         230 /* For non-TPM type sensors, set to line voltage,
such as 240; For Efergy Elite 3.0 TPM,  set to 1 */
#define TARGET_UID 0x0b18bc /* The first 3 bytes of the frame received is used
to uniquely identify the transmitter. If set, only frames match this UID will be
captured; otherwise, set to -1 to disable this feature */

#define FRAMEBYTECOUNT          8  /* Attempt to decode up to this many bytes. */
...
#define APPROX_SAMPLES_PER_BIT  20
...
void display_frame_data(int debug_level, char *msg, unsigned char bytes[], int bytecount) {
    ...
    if (bytecount < 3) {
        ...
        int curr_UID = bytes[0]*65536+bytes[1]*256+bytes[2];
    ...
    if (debug_level > 0) {
        ...
        printf("total_energy_consumption value=%f %u000000000\n", result, (unsigned)time(NULL));
        if (loggingok) {
            if (LOGTYPE) {
                fprintf(fp, "%s,%f\r\n", buffer, result);
            } else {
...
int main(int argc, char**argv)
{
    ...
    while ( !feof(stdin) ) {
        ...
        while ( !feof(stdin) ) {
            ...
            if (sample_store_index < SAMPLE_STORE_SIZE)
                sample_store_index++;
            else {
